...
-- Note: Replace 'admin-uuid' with actual UUID from auth.users after first signup
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM factory_app.user_profiles WHERE role = 'admin') THEN
        INSERT INTO factory_app.user_profiles (id, full_name, role, department)
        VALUES (
