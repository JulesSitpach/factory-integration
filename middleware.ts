...
export async function middleware(request: NextRequest) {
  ...
  if (pathname === '/') {
    ...
    return NextResponse.redirect(new URL(`/marketing/${preferredLocale}`, request.url));
  }
  
  // Handle direct access to /marketing without locale
  if (pathname === '/marketing' || pathname.startsWith('/marketing/') && !pathnameHasLocale) {
    // Get the preferred locale from the Accept-Language header
    const acceptLanguage = request.headers.get('accept-language') || '';
    const preferredLocale = acceptLanguage.includes('es') ? 'es' : defaultLocale;
    
    // Redirect to the marketing page with the preferred locale
    const newPath = pathname === '/marketing' 
      ? `/marketing/${preferredLocale}` 
      : `/marketing/${preferredLocale}${pathname.substring('/marketing'.length)}`;
    
    return NextResponse.redirect(new URL(newPath, request.url));
  }
  
  // Handle direct access to /dashboard without locale
  if (pathname === '/dashboard' || (pathname.startsWith('/dashboard/') && !pathname.match(/^\/dashboard\/[a-z]{2}\/?/))) {
