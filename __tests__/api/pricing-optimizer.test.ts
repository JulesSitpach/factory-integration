...
const mockOptimizationResult = {
  ...
  product: mockProductData,
  target_margin: 30,
  scenarios: [mockScenarioResult, { ...mockScenarioResult, scenario_name: 'Tariff Hike 10%' }],
  recommendations: {
  ...
  sensitivity_analysis: {
    margin_impact_by_price: [{ price: 90, margin: 25 }, { price: 100, margin: 30 }, { price: 110, margin: 35 }],
    volume_impact_by_price: [{ price: 90, volume: 1100 }, { price: 100, volume: 1000 }, { price: 110, volume: 900 }],
    profit_impact_by_price: [{ price: 90, profit: 27500 }, { price: 100, profit: 30000 }, { price: 110, profit: 31500 }],
  },
...
describe('Pricing Optimizer API', () => {
  ...
  beforeEach(() => {
    ...
    mockSupabaseClient = {
      ...
      insert: jest.fn().mockResolvedValue({ data: { id: 'hist-entry-123' }, error: null }),
    };
  ...
  describe('Authentication checks', () => {
    it('should return 401 if user is not authenticated', async () => {
      (auth.getAuth as jest.Mock).mockResolvedValue({
        session: null, user: null, error: null, isAuthenticated: false,
      });
  ...
  describe('Optimization Logic', () => {
    ...
    it('should generate price points based on strategies and price range', async () => {
        ...
        const baseCaseScenario = responseBody.scenarios.find((s:any) => s.scenario_name === 'Base Case');
        ...
        baseCaseScenario.price_points.forEach((pp: any) => {
            ...
            if (mockOptimizationRequest.price_range.min) {
                expect(pp.price).toBeGreaterThanOrEqual(mockOptimizationRequest.price_range.min - 0.01);
            }
            if (mockOptimizationRequest.price_range.max) {
                expect(pp.price).toBeLessThanOrEqual(mockOptimizationRequest.price_range.max + 0.01);
            }
        });
      });
    ...
    it('should use existing optimization results if recent', async () => {
        ...
        mockSupabaseClient.limit.mockResolvedValue({ data: [recentResult], error: null });

        const request = createMockRequest(mockOptimizationRequest);
        await POST(request);

        expect(NextResponse.json).toHaveBeenCalledWith(recentResult);
        expect(mockSupabaseClient.insert).not.toHaveBeenCalledWith(expect.objectContaining({ from: 'pricing_optimizations'})); 
    });
    ...
    it('should re-calculate if existing optimization results are outdated', async () => {
        ...
        mockSupabaseClient.limit.mockResolvedValue({ data: [outdatedResult], error: null });
        ...
        expect(mockSupabaseClient.from).toHaveBeenCalledWith('pricing_optimizations');
  ...
  describe('Error Handling', () => {
    it('should handle database errors when fetching existing optimizations', async () => {
      mockSupabaseClient.limit.mockResolvedValue({ data: null, error: new Error('DB fetch error') });
    ...
    it('should handle database errors when storing optimization results', async () => {
      mockSupabaseClient.insert.mockResolvedValueOnce({ data: null, error: new Error('DB insert error for optimizations') });
      // Second insert is for history
      mockSupabaseClient.insert.mockResolvedValueOnce({ data: null, error: new Error('DB insert error for history') });
    ...
    it('should return 500 for unexpected errors during optimization', async () => {
        ...
        const request = {
            json: jest.fn().mockRejectedValue(new Error('Unexpected internal error')),
          } as unknown as NextRequest;
  ...
  describe('History Logging', () => {
    it('should log optimization history in the database', async () => {
      ...
      expect(mockSupabaseClient.from).toHaveBeenCalledWith('user_optimization_history');
