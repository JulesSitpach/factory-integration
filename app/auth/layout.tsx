    ...
    <div className="min-h-screen bg-secondary flex flex-col">
      ...
      <header className="bg-white shadow-sm py-4">
        <div className="container mx-auto px-6 flex justify-between items-center">
          <Link href={`/marketing/${locale}`} className="flex items-center space-x-2 no-underline">
            <div className="w-10 h-10 bg-primary rounded-md flex items-center justify-center">
              <span className="text-white font-bold text-xl">T</span>
            </div>
            <span className="text-primary font-semibold text-xl">TradeNavigatorPro</span>
          </Link>
          ...
          <div className="relative group">
            ...
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-50">
              <div className="py-1">
                <Link 
                  href={`/auth${window?.location?.pathname?.replace(`/auth`, '')?.replace(`/${locale}`, '')}/en`} 
                  className="block px-4 py-2 text-sm text-slate hover:bg-secondary no-underline"
                >
                  {t.english}
                </Link>
                <Link 
                  href={`/auth${window?.location?.pathname?.replace(`/auth`, '')?.replace(`/${locale}`, '')}/es`}
                  className="block px-4 py-2 text-sm text-slate hover:bg-secondary no-underline"
                >
                  {t.spanish}
                </Link>
        ...
        <div className="container mx-auto px-6">
              ...
              <div className="w-full md:w-1/2 bg-primary text-white p-8 md:p-12 flex flex-col">
                ...
                <div className="mt-8 pt-8 border-t border-white/20">
                  ...
                  <div className="flex gap-4 mt-4">
                    ...
                    {[1, 2, 3].map((i) => (
                      <div key={i} className="h-8 w-16 bg-white/20 rounded-md" />
                    ))}
                  </div>
                </div>
              </div>
          ...
          <div className="mt-8 text-center text-sm text-slate/70">
            <div className="space-x-4">
              <Link href={`/marketing/${locale}`} className="hover:text-primary">
                {t.backToHome}
              </Link>
              <span>•</span>
              <Link href={`/marketing/${locale}/contact`} className="hover:text-primary">
                {t.contactSupport}
              </Link>
              <span>•</span>
              <Link href={`/marketing/${locale}/legal/privacy`} className="hover:text-primary">
                {t.privacyPolicy}
              </Link>
              <span>•</span>
              <Link href={`/marketing/${locale}/legal/terms`} className="hover:text-primary">
                {t.termsOfService}
              </Link>
            </div>
          </div>
