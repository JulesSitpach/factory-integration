...
const productDataSchema = z.object({
  ...
  minimum_viable_price: z.number().nonnegative('Minimum viable price must be non-negative'),
  competitor_prices: z.array(z.number().nonnegative()).optional(),
  price_elasticity: z.number().optional(),
  sales_volume_current: z.number().positive('Current sales volume must be positive'),
...
export async function POST(request: NextRequest) {
  try {
    ...
    await logOptimizationHistory(auth.session.user.id, optimizationRequest, result.id, supabase);
...
async function optimizePricing(
  ...
  const { product, target_margin, scenarios, price_range, strategies } = request;
  ...
  if (!error && existingData && existingData.length > 0) {
    ...
    const now = new Date();
    const hoursSinceLastOptimization = (now.getTime() - resultDate.getTime()) / (1000 * 60 * 60);
  ...
  const scenarioResults = scenarios.map(scenario => 
    calculateScenarioResult(product, scenario, target_margin, selectedStrategies, price_range)
  );
  
  // Generate overall recommendations
  const recommendations = generateRecommendations(product, scenarioResults, target_margin);
  
  // Generate sensitivity analysis
  const sensitivityAnalysis = generateSensitivityAnalysis(product, target_margin);
...
function calculateScenarioResult(
  ...
  const minPrice = priceRange?.min || Math.max(breakEvenPrice, product.minimum_viable_price);
  ...
  strategies.forEach(strategy => {
    ...
    const totalCost = (adjustedCosts.total_unit_cost * volumeProjection) + 
    ...
    if (product.market_share_current !== undefined && strategy.market_share_projection_factor) {
      marketShareProjection = product.market_share_current * 
    ...
    pricePoints.push({
      ...
      price_change_percentage: ((strategicPrice - product.current_price) / product.current_price) * 100,
  ...
  for (let price = minPrice; price <= maxPrice; price += step) {
    ...
    pricePoints.push({
      ...
      volume_projection: volumeProjection,
      market_share_projection: marketShareProjection,
      price_change_percentage: ((price - product.current_price) / product.current_price) * 100,
      is_recommended: Math.abs(margin - targetMargin) < 5, // Within 5% of target margin
    });
  }
  ...
  const optimalPrice = pricePoints.length > 0 ? pricePoints[0].price : product.current_price;
  const optimalMargin = pricePoints.length > 0 ? pricePoints[0].margin_percentage : 0;
  ...
  if (product.competitor_prices && product.competitor_prices.length > 0) {
    const avgCompetitorPrice = product.competitor_prices.reduce((sum, price) => sum + price, 0) / 
                              product.competitor_prices.length;
    
    const priceDiffPercentage = ((optimalPrice - avgCompetitorPrice) / avgCompetitorPrice) * 100;
  ...
  if (optimalPrice > product.current_price * 1.2) {
    riskFactors.push('Significant price increase may reduce customer retention');
    riskLevel = 'medium';
  }
  ...
  if (optimalMargin < 10) {
    riskFactors.push('Low profit margin increases vulnerability to cost fluctuations');
    riskLevel = riskLevel === 'high' ? 'high' : 'medium';
  }
  
  // Check competitor positioning
  if (competitorComparison) {
    if (competitorComparison.relative_position === 'higher' && 
        competitorComparison.price_difference_percentage > 15) {
      riskFactors.push('Price significantly higher than competitors may reduce market share');
      riskLevel = riskLevel === 'high' ? 'high' : 'medium';
    }
  }
 ...
 */
function calculateAdjustedCosts(product: ProductData, scenario: ScenarioParameter) {
...
function calculateBreakEvenPrice(costs: {
  ...
  return costs.total_unit_cost + costs.variable_costs + (costs.fixed_costs / assumedVolume);
}
 ...
 */
function calculateMargin(price: number, costs: {
  total_unit_cost: number;
  variable_costs: number;
}): number {
 ...
 */
function calculatePriceForMargin(costs: {
  total_unit_cost: number;
  variable_costs: number;
}, targetMargin: number): number {
  const totalUnitCost = costs.total_unit_cost + costs.variable_costs;
...
function calculateVolumeProjection(
  ...
  const newVolume = currentVolume * (1 + volumeChangePercent) * adjustmentFactor;
...
function generateRecommendations(
  ...
  if (bestPricePoint.price > product.current_price * 1.05) {
    insights.push(`Price increase of ${bestPricePoint.price_change_percentage.toFixed(1)}% is recommended to optimize profitability`);
  } else if (bestPricePoint.price < product.current_price * 0.95) {
    insights.push(`Price decrease of ${Math.abs(bestPricePoint.price_change_percentage).toFixed(1)}% is recommended to optimize profitability`);
  } else {
    insights.push("Current pricing is close to optimal");
  }
  
  // Margin insight
  if (bestPricePoint.margin_percentage < targetMargin - 5) {
    insights.push(`Achieving target margin of ${targetMargin}% may require cost optimization`);
  } else if (bestPricePoint.margin_percentage > targetMargin + 5) {
    insights.push(`Margin exceeds target by ${(bestPricePoint.margin_percentage - targetMargin).toFixed(1)}%, consider competitive pricing to gain market share`);
  }
  
  // Volume insight
  if (bestPricePoint.volume_projection < product.sales_volume_current * 0.9) {
    insights.push("Expected volume decrease may require operational adjustments");
  } else if (bestPricePoint.volume_projection > product.sales_volume_current * 1.1) {
    insights.push("Prepare for increased production volume to meet projected demand");
  }
  
  // Risk insight
  if (bestScenario.risk_assessment.level === 'high') {
    insights.push("High risk strategy: Consider phased implementation and close monitoring");
  } else if (bestScenario.risk_assessment.level === 'medium') {
    insights.push("Medium risk strategy: Monitor key performance indicators closely");
  }
  
  // Competitor insight
  if (bestScenario.competitor_comparison) {
    if (bestScenario.competitor_comparison.relative_position === 'higher') {
      insights.push(`Price will be ${bestScenario.competitor_comparison.price_difference_percentage.toFixed(1)}% higher than competitors, emphasize value proposition`);
    } else if (bestScenario.competitor_comparison.relative_position === 'lower') {
      insights.push(`Price will be ${Math.abs(bestScenario.competitor_comparison.price_difference_percentage).toFixed(1)}% lower than competitors, potential to gain market share`);
    }
  }
  
  // Determine optimal strategy name
  let optimalStrategy = "Balanced Pricing";
  if (bestPricePoint.price > product.current_price * 1.1) {
    optimalStrategy = "Premium Positioning";
  } else if (bestPricePoint.price < product.current_price * 0.9) {
    optimalStrategy = "Competitive Pricing";
  }
  
  return {
    optimal_strategy: optimalStrategy,
    price_suggestion: bestPricePoint.price,
    expected_margin: bestPricePoint.margin_percentage,
    expected_profit: bestPricePoint.profit,
    expected_revenue: bestPricePoint.revenue,
 ...
 */
function generateSensitivityAnalysis(product: ProductData, targetMargin: number) {
  ...
  const baseCosts = {
    ...
    total_unit_cost: product.unit_cost + (product.unit_cost * (product.tariff_rate / 100)) + product.shipping_cost,
  ...
  for (let price = minPrice; price <= maxPrice; price += step) {
    ...
    const totalCost = (baseCosts.total_unit_cost * volume) + 
...
async function logOptimizationHistory(
  ...
  try {
    await supabase
      .from('user_optimization_history')
      .insert({
        user_id: userId,
...
function getAvailablePricingStrategies(): PricingStrategy[] {
  return [
    ...
    },
    {
      ...
      recommended_for: ["Premium Products", "Unique Solutions", "Branded Goods"],
    },
    {
      ...
      recommended_for: ["Commoditized Markets", "High Competition", "Market Entry"],
    },
    {
    ...
    },
    {
      ...
      recommended_for: ["Luxury Goods", "High-End Products", "Exclusive Services"],
    },
    {
      name: "Skimming",
      ...
      recommended_for: ["Innovative Products", "Early Adopter Markets", "Limited Competition"],
    },
    {
      ...
      recommended_for: ["Basic Products", "Price Sensitive Markets", "High Volume"],
    },
    {
    ...
    },
    {
      ...
      recommended_for: ["Complementary Products", "Service Packages", "Cross-Selling"],
    },
    {
      ...
      recommended_for: ["E-commerce", "Seasonal Products", "High Demand Variation"],
    },
  ];
}
...
function generateUniqueId(): string {
  return 'opt-' + Math.random().toString(36).substring(2, 15) + 
