...
export default function EmergencyCostCalculator() {
  ...
  const prepareCostBreakdownChart = () => {
    ...
    return {
      ...
      datasets: [
        {
          data: [
            summary.total_base_cost,
            summary.total_tariff_cost,
          ],
  ...
  const prepareTariffByCountryChart = () => {
    ...
    calculationResult.details.forEach((detail) => {
      const existing = countryMap.get(detail.country_of_origin) || { tariffCost: 0, count: 0 };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-lg font-semibold mb-4">{t('uploadPurchaseOrder')}</h2>
      ...
      {calculationResult && (
        <div className="bg-white rounded-lg shadow p-6">
          <h2 className="text-lg font-semibold mb-4">{t('calculationResults')}</h2>
          
          {/* Summary Cards */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
              <h3 className="text-sm text-blue-700 font-medium">{t('totalProducts')}</h3>
              <p className="text-2xl font-bold">{calculationResult.summary.total_products}</p>
            </div>
            
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
              <h3 className="text-sm text-blue-700 font-medium">{t('totalLandedCost')}</h3>
            ...
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
              <h3 className="text-sm text-blue-700 font-medium">{t('averageTariffRate')}</h3>
              <p className="text-2xl font-bold">
                {formatPercentage(calculationResult.summary.average_tariff_rate)}
              </p>
            </div>
            
            <div className="bg-green-50 p-4 rounded-lg border border-green-100">
              <h3 className="text-sm text-green-700 font-medium">{t('potentialSavings')}</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              <h3 className="text-sm font-medium mb-3">{t('tariffByCountry')}</h3>
              <div className="h-64">
                ...
                {calculationResult.details.map((detail, index) => (
                  <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                      {detail.product_name}
                      <div className="text-xs text-gray-500">{detail.product_code}</div>
                    </td>
          ...
          <div className="mt-8">
            <h3 className="text-md font-medium mb-3">{t('alternativeSuppliers')}</h3>
            <div className="bg-green-50 border border-green-100 rounded-lg p-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-600 mt-1">
                    {t('savingsDescription', { 
                      amount: formatCurrency(calculationResult.summary.potential_savings * calculationResult.summary.total_base_cost / 100)
                    })}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {calculationResult.details.map((detail, index) => (
                  detail.alternative_suppliers && detail.alternative_suppliers.length > 0 && (
                    <div key={index} className="bg-white p-3 rounded border border-gray-200">
                      <h4 className="font-medium text-sm">{detail.product_name}</h4>
                      <p className="text-xs text-gray-500 mb-2">
                        {t('currentSupplier')}: {detail.country_of_origin} ({formatPercentage(detail.tariff_rate)})
                      </p>
                      <ul className="space-y-1">
                        {detail.alternative_suppliers.map((supplier, supplierIndex) => (
                          <li key={supplierIndex} className="text-xs flex justify-between">
                            <span>{supplier.country}</span>
                            <span className="text-green-600">
                              {formatPercentage(supplier.tariff_rate)} ({supplier.potential_savings > 0 ? '-' : '+'}
                              {formatPercentage(Math.abs(supplier.potential_savings))})
                            </span>
                          </li>
                        ))}
                      </ul>
