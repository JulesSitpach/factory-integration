...
export default function CalculatorPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : t("calculator.errors.unknownError"));
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-green-700">
                <span className="font-medium">{t("calculator.totalCost")}:</span> {formatCurrency(result)}
              </p>
              <div className="mt-2 text-sm text-green-700">
                <ul className="list-disc pl-5 space-y-1">
                  <li>{t("calculator.materials")}: {formatCurrency(Number(materials))}</li>
                  <li>{t("calculator.labor")}: {formatCurrency(Number(labor))}</li>
                  <li>{t("calculator.overhead")}: {formatCurrency(Number(overhead))}</li>
                </ul>
              </div>
            </div>
          </div>
