...
const translations = {
  ...
  es: {
    ...
    dashboardSubtitle: 'Esto es lo que está sucediendo con sus operaciones comerciales',
...
export default async function DashboardPage({ params }: { params: { locale: string } }) {
  ...
  const profileCompletion = profile?.onboarding_completed ? 100 : 
    (profile?.full_name && profile?.company_name && profile?.industry) ? 75 : 
    (profile?.full_name) ? 50 : 25;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* Summary Metrics */}
      <div>
        <h2 className="text-lg font-semibold text-slate mb-3">{t.summaryMetrics}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {/* Tariff Impact */}
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate/70">{t.tariffImpact}</p>
                <p className="text-2xl font-bold text-slate mt-1">{tariffImpact}</p>
              </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate/70">{t.potentialSavings}</p>
                <p className="text-2xl font-bold text-slate mt-1">{potentialSavings}</p>
              </div>
            ...
            <div className="mt-4 flex items-center">
              ...
              <span className="text-slate/60 text-sm ml-2">opportunities identified</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate/70">{t.riskLevel}</p>
          ...
          <div className="bg-white rounded-lg shadow p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-slate/70">{t.supplierCount}</p>
                <p className="text-2xl font-bold text-slate mt-1">{supplierCount}</p>
              </div>
      ...
      {/* Quick Actions */}
      <div>
        <h2 className="text-lg font-semibold text-slate mb-3">{t.quickActions}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
          {/* Emergency Cost Calculator */}
          <Link href={`/dashboard/${params.locale}/calculator`} className="bg-white rounded-lg shadow p-6 hover:shadow-md transition-shadow no-underline">
          ...
          <Link href={`/dashboard/${params.locale}/planner`} className="bg-white rounded-lg shadow p-6 hover:shadow-md transition-shadow no-underline">
          ...
          <Link href={`/dashboard/${params.locale}/optimizer`} className="bg-white rounded-lg shadow p-6 hover:shadow-md transition-shadow no-underline">
          ...
          <Link href={`/dashboard/${params.locale}/tracker`} className="bg-white rounded-lg shadow p-6 hover:shadow-md transition-shadow no-underline">
          ...
          <Link href={`/dashboard/${params.locale}/router`} className="bg-white rounded-lg shadow p-6 hover:shadow-md transition-shadow no-underline">
            <div className="w-12 h-12 bg-primary/10 rounded-lg flex items-center justify-center mb-4">
              <svg className="w-6 h-6 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"></path>
              </svg>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-lg font-semibold text-slate">{t.recentActivity}</h2>
            <Link href={`/dashboard/${params.locale}/activity`} className="text-sm text-primary hover:underline">
              {t.viewAll}
            </Link>
          </div>
            ...
            {sampleActivities.length > 0 ? (
              <ul className="divide-y divide-gray-200">
                {sampleActivities.map((activity) => (
                  <li key={activity.id} className="p-4 hover:bg-secondary transition-colors">
                    <div className="flex items-center">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-slate/60">
                          {formatRelativeTime(activity.timestamp, params.locale)}
                        </p>
                      </div>
                    </div>
                  </li>
        ...
        <div>
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-lg font-semibold text-slate">{t.alertsTitle}</h2>
          </div>
          <div className="bg-white rounded-lg shadow">
            {sampleAlerts.length > 0 ? (
              <ul className="divide-y divide-gray-200">
                {sampleAlerts.map((alert) => (
                  <li key={alert.id} className="p-4 hover:bg-secondary transition-colors">
                    <div className="flex items-center">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex flex-col md:flex-row items-center justify-between">
            <div className="flex-1">
              <h3 className="text-lg font-medium text-slate">{t.profileCompletion}</h3>
